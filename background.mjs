...
/**
 * Multi AI Assistant — Background Service Worker (ESM)
 *
 * Entry point for the extension's background service worker.
 * Uses ES module static imports (not the legacy script-loading pattern).
 */
...
import { createSessionRegistry } from "./session/session-registry.mjs";
import {
  SESSION_PROVIDER_IDS as BINDING_PROVIDER_IDS,
  normalizeChildSessionBinding,
  shouldIgnoreChildSessionUrl,
  isSessionProviderSupported
} from "./session/provider-session-bindings.mjs";
import {
...
function getSessionProviderIds() {
  if (Array.isArray(BINDING_PROVIDER_IDS) && BINDING_PROVIDER_IDS.length > 0) {
    return BINDING_PROVIDER_IDS.slice();
  }
  if (Array.isArray(SESSION_PROVIDER_IDS) && SESSION_PROVIDER_IDS.length > 0) {
    return SESSION_PROVIDER_IDS.slice();
  }
...
function sanitizeChildSessionRecord(provider, child) {
  ...
  const ignored =
    typeof shouldIgnoreChildSessionUrl === "function" &&
    shouldIgnoreChildSessionUrl(provider, existing.url);
  ...
  if (typeof normalizeChildSessionBinding === "function") {
    const normalized = normalizeChildSessionBinding({
      provider,
      url: existing.url,
...
async function handleSessionRestore(sessionId) {
  ...
  const restorePlan = normalizeRestorePlan(session);
...
async function handleSessionSyncChild(message, sender) {
  ...
  const supported =
    typeof isSessionProviderSupported === "function"
      ? isSessionProviderSupported(provider)
  ...
  if (
    typeof shouldIgnoreChildSessionUrl === "function" &&
    shouldIgnoreChildSessionUrl(provider, message?.url)
  ) {
