...
/**
 * Providers Module (ESM)
 *
 * Provider definitions and lookup utilities for the background service worker.
 */

export const DASHBOARD_MAX_PANELS = Infinity;
...
export const PROVIDER_BY_ID = PROVIDERS.reduce((acc, provider) => {
  acc[provider.id] = provider;
  return acc;
}, {});

export function normalizeProviders(list, maxCount) {
  const seen = new Set();
  const result = [];
...
export function findProviderByToken(token) {
