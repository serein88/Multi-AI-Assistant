...
- 时间：2026-06-29
- 任务：T-20260622-023 稳定性改进：消息传递超时保护
- 状态：待确认
- 变更文件：
  - `content/constants.js` — 新增 RUNTIME_MESSAGE_TIMEOUT_MS(5000), RUNTIME_MESSAGE_RETRY_COUNT(3), RUNTIME_MESSAGE_RETRY_DELAY_MS(250)
...
- 状态：待确认
- 变更文件：
  - `content/send-handlers.js` — 新增 SHADOW_DENY_PATTERNS（19 条敏感模式）、SHADOW_ALLOW_PATTERNS（18 条 AI 组件模式）、isSensitiveShadowHost()、shouldTraverseShadowHost()；deepQueryAll 和 Copilot 局部 deepQuery 均加入 shouldTraverseShadowHost 守卫；导出到 `globalThis.__MAI_Send`
  - `tests/content/shadow-dom-traversal.test.js` — 新建，26 项测试
- 设计决策：
  ...
  - denylist 优先，allowlist 匹配不区分 custom element 与标准 HTML 元素 — 标准元素（如 `<div class="chat-input-container">`）命中 allowlist 时同样可遍历
  - `/toolbar/i` 已移除，改为更窄的 `/send[\s_-]?toolbar/i` 和 `/composer[\s_-]?toolbar/i`，避免普通 toolbar 组件被放行
...
- 时间：2026-06-25
- 任务：T-20260622-017 验收标准确认 → 待确认
- 状态：待确认
- 决策：用户选择选项 1 — 调整验收标准
- 新验收标准：
  1. 关键路径集成测试覆盖（trySendPrompt、background routing、session create/restore/transcript）✅
  2. 覆盖率基线防回退 ≥ 28%（当前 28.81%）✅
  3. E2E 核心链路测试（发送→响应→转录）— 可选，作为后续任务
- 原验收标准（已弃用）：覆盖率 60%
- 弃用原因：MV3 + DOM + Chrome API + ESM/legacy 双文件架构导致 coverage 分母失真，继续补单元测试边际收益极低
- 已交付：
  - 414 项测试通过（83 项新增）
  - 关键路径集成测试：trySendPrompt (36)、background routing (10)、session (7)
  - 覆盖率基线 28.81%
  - 测试基础设施修正（lint 覆盖、eslint 配置）
- T-017 改为"待确认"，等待用户确认后改为"完成"
- 后续建议：E2E 核心链路测试（发送→响应状态→transcript 写入）作为新任务
